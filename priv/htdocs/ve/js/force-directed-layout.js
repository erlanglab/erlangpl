  ...
  this.generate = function() {
    if(layout_iterations < this.max_iterations && temperature > 0.000001) {
      ...
      for(var i=0; i < nodes_length; i++) {
        ...
        for(var j=i+1; j < nodes_length; j++) {
          ...
          if(i != j) {
            ...
            var delta_length = 0;
            if(this.layout === "3d") {
              delta_length = Math.max(EPSILON, Math.sqrt((delta_x * delta_x) + (delta_y * delta_y) + (delta_z * delta_z)));
            }
            else {
              delta_length = Math.max(EPSILON, Math.sqrt((delta_x * delta_x) + (delta_y * delta_y)));
            }
            
            //delta_length = Math.max(EPSILON, delta_length - 2 * node_u.data.size - 2 * node_v.data.size);

            var force = (repulsion_constant * repulsion_constant) / (delta_length * delta_length);
            //force += node_v.data.size;
            //force += node_u.data.size;

            node_v.layout.force += force;// * node_v.data.size;
            node_u.layout.force += force;// * node_u.data.size;

            var offset_x = (delta_x / delta_length) * force;
            var offset_y = (delta_y / delta_length) * force;

            node_v.layout.offset_x += offset_x;// * node_v.data.size;
            node_v.layout.offset_y += offset_y;// * node_v.data.size;
            ...
            node_u.layout.offset_x -= offset_x;// * node_u.data.size;
            node_u.layout.offset_y -= offset_y;// * node_u.data.size;

            if(this.layout === "3d") {
              var offset_z = (delta_z / delta_length) * force;
              node_v.layout.offset_z += offset_z;// * node_v.data.size;
              node_u.layout.offset_z -= offset_z;// * node_u.data.size;
            }
          }
        }
      }
      ...
      for(var i=0; i < nodes_length; i++) {
        ...
        if(this.layout === "3d") {
          delta_length = Math.max(EPSILON, Math.sqrt(node.layout.offset_x * node.layout.offset_x + node.layout.offset_y * node.layout.offset_y + node.layout.offset_z * node.layout.offset_z));
        }
        else {
